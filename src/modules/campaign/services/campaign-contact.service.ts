...
import { CampaignRun, CampaignRunDocument } from '../schemas/campaign-run.schema';
...
export class CampaignContactService {
    ...
    constructor(
        ...
        @InjectModel(CampaignRun.name) private readonly campaignRunModel: Model<CampaignRunDocument>,
  ...
  try {
    ...
    const campaignRun =
      await this.campaignRunModel.findById(
        campaignRunId,
      );
    ...
    // get template
    const template =
      await this.templateService.getTemplateById(
  ...
  async deleteContacts(campaignRunId: string): Promise<string> {
    try{
      ...
      const campaignRunExists = await this.campaignRunModel.exists({_id: campaignRunId, isDeleted: false});
      if(!campaignRunExists){
        throw new NotFoundException('Campaign run not found');
      }
      await this.campaignContactModel.deleteMany(
        { campaignRunId, isDeleted: false }
      );
